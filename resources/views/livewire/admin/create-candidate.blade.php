...
<div class="col-span-full">
    ...
    <div class="mt-2 flex justify-center">
        @if($photo)
            <!-- Preview saat ada file (baru atau existing) -->
            <div class="relative">
                <img src="{{ 
                ...
                <button 
                    ...
                    class="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-1 hover:bg-red-600 transition-all"
                    title="Hapus foto">
        ...
        @else
            <!-- Placeholder upload -->
            <label for="photo" class="cursor-pointer flex flex-col items-center justify-center px-2 py-4 border-2 border-dashed border-gray-300 rounded-lg hover:border-indigo-400 transition-colors">
                ...
                <span class="mt-2 text-xs text-gray-500 text-center">Klik untuk upload</span>
                <input 
                    type="file" 
                    id="photo" 
                    wire:model="photo" 
    ...
    @if($photo instanceof \Livewire\TemporaryUploadedFile)
        <div class="mt-2 text-center">
            <svg aria-hidden="true" class="w-5 h-5 text-indigo-500 animate-spin mx-auto" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <span class="text-xs text-gray-500 mt-1">Mengupload...</span>
        </div>
    @endif
    ...
    <p class="mt-2 text-xs text-gray-500 text-center">Format: JPG/PNG, maks. 2MB</p>
</div>
    ...
    <div class="w-full lg:w-1/2">
        <div class="bg-white rounded-xl shadow-md p-6 h-fit">
            <h2 class="text-2xl font-bold text-gray-800 mb-4">Daftar Kandidat</h2>
...
<!-- Optional: Alert via JS -->
@push('scripts')
<script>
    document.addEventListener('DOMContentLoaded', function () {
        window.addEventListener('alert', event => {
            const { type, message } = event.detail;
            // Kamu bisa integrasikan dengan Toast, misalnya:
            alert(message); // ganti dengan toast library jika ada
        });
    });
</script>
@endpush